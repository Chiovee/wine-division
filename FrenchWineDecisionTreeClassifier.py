...
for column_index, column in enumerate(df.columns[1:6]):
...
wine_test = [[13.52,3.17,2.72,23.5,97.],
             [12.42,2.55,2.27,22. ,90.],
             [13.76,1.53,2.7 ,19.5,132.]]
list1 = model.predict(wine_test)
#print(model.predict(wine_test))
for i in range(0,len(list1)):#输出测试的结果，即输出模型预测的结果
	if list1[i] == 'Zinfandel':
		list1[i] = '仙粉黛'
	elif list1[i] == 'Syrah':
		list1[i] = '西拉'
	else:
